...
namespace Naticron
{
	...
	{
		private static readonly Regex _dayPattern = new Regex(
			@"^\d\d?$",
			RegexOptions.Singleline | RegexOptions.Compiled);

		private static readonly string[] _dayPeriods = {"am", "pm", "morning", "afternoon", "evening", "night"};

		private static readonly Regex _monthPattern = _dayPattern;

		private static readonly Regex _pattern = new Regex(
			@"^\d*$",
			RegexOptions.Singleline | RegexOptions.Compiled);

		private static readonly Regex _yearPattern = new Regex(
			@"^([1-9]\d)?\d\d?$",
			RegexOptions.Singleline | RegexOptions.Compiled);
		...
		private static bool TokenIsAPeriodOfDay(Token token) => token != null && _dayPeriods.Contains(token.Value);
	}
}