...
namespace Naticon
{
	public static class TokenCollectionExtensions
	{
			...
			this IEnumerable<Token> @this, Options options)
		{
			...
			if (options.AmbiguousTimeRange != 0)
			{
				...
				for (var i = 0; i < tokenSize; i++)
				{
					...
					var nextToken = i < tokenSize - 1 ? tokens[i + 1] : null;
			...
			Options options)
		{
			...
			var pointer = Pointer.Type.Future;
			var tokens = @this.ToList();

			var repeaters = tokens.GetRepeaters();
			...
			var head = repeaters[0];
			repeaters.RemoveAt(0);

			head.Now = options.Clock();

			Span outerSpan;
			var grabberType = grabber.Value;
			...
			Logger.Log(() => "--" + outerSpan);
			var anchor = repeaters.FindWithin(outerSpan, pointer, options);
			return anchor;
		}
